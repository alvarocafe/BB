...
function cluster(X, max_elem)
    ...
    return  Tree,child,center_row,diam,inode,ileaf
end
#############################################################################################################################
function checa_admiss(η,center_row,diam,inode,ileaf)
...
function divnode(X,t)
    ...
    F  = eigen(covx)          # Calucla autovalores e autovetores da matriz de covariancia de x
    eig_valx=F.values
    eig_vecx=F.vectors
    valxmax,indxmax=findmax(eig_valx)
    ref = eig_vecx[:,indxmax]      # Define como referencia o autovetor relacionado ao maior autovalor
