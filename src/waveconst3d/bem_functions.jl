...
function calcula_HeGns_POT(x1,y1,z1,x2,y2,z2,x3,y3,z3,xd,yd,zd,n,qsil,w,FR,CW)
n_pint=length(qsil);
eta = qsil;
rho = w;
Tast = complex(0,0);
...
for l=1:n_pint
    for m=1:n_pint
        ...
        Tast,qast=calc_solfund_POT(x,y,z,xd,yd,zd,n,FR,CW);
...
for kk=1:3
    ...
    for ii = 1: npg # la�o sobre a primeira vari�vel de integra��o
        for jj = 1: npg # la�o sobre a segunda vari�vel de integra��o
            ...
            Tast,qast = calc_solfund_POT(xc, yc, zc,xd,yd,zd,[0 0 0], FR,CW) # Sol.
...
function calc_solfund_POT(x,y,z,xd,yd,zd,n,FR,CW)
rx=x-xd;
ry=y-yd;
rz=z-zd;
