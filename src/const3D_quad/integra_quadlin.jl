...
p3 = [1 1 0]
p4 = [0 1 0]
NOS_GEO,NOS,ELEM,CDC = dad_quadlin(p1,p2,p3,p4)
...
for i = 1:size(qsi,2)
  for j = 1:size(qsi,2)
    N[1,:] =calc_fforma_quadlin(qsi[i],qsi[j]); # Calcula as funções de forma
    dN[1,:] = calc_dfforma_quadlin(qsi[i],qsi[j])
    x[i,j]=N[1,1]*NOS_GEO[1,2]+N[1,2]*NOS_GEO[2,2]+N[1,3]*NOS_GEO[3,2]+N[1,4]*NOS_GEO[4,2]; # Calcula a coordenada x do ponto de integração
    y[i,j]=N[1,1]*NOS_GEO[1,3]+N[1,2]*NOS_GEO[2,3]+N[1,3]*NOS_GEO[3,3]+N[1,4]*NOS_GEO[4,3]; # Calcula a coordenada y do ponto de integração
    z[i,j]=N[1,1]*NOS_GEO[1,4]+N[1,2]*NOS_GEO[2,4]+N[1,3]*NOS_GEO[3,4]+N[1,4]*NOS_GEO[4,4]; # Calcula a coordenada y do ponto de integração
    dgamadqsi[i,j]=cal_Jacobiano3D_quadlin(NOS_GEO[:,2], NOS_GEO[:,3], NOS_GEO[:,4], dN[1,:]); # Jacobiano
...
title("Quadrilateral bilinear element",fontsize="16.0")
#legend(fontsize="14.0",loc="best")
PyPlot.xlim(-0.2, 1.2)
PyPlot.ylim(-0.2, 1.2)
#axis("equal")
savefig("docs/discretização/figuras/quadconstante.pdf",transparent = true)
